...
async def test_orchestrator_with_dependencies(mock_table, mock_args):
    ...
    modules_to_run = ["ptr"]
    with patch('modules.orchestrator._create_execution_plan',
               return_value=['records', 'ptr']):
        await _scan_single_domain("example.com", mock_args, modules_to_run)

    # Check that both analysis functions were called
    mock_table.items.return_value[0][1]['analysis_func'].assert_awaited_once() # noqa: E501
    mock_table.items.return_value[1][1]['analysis_func'].assert_awaited_once() # noqa: E501
...
async def test_orchestrator_selective_run(mock_table, mock_args):
    ...
    """
    mock_table.get.side_effect = {
        "records": {"analysis_func": AsyncMock(), "display_func": MagicMock(),
                    "dependencies": [], "data_key": "records", "description": "..."},
        "whois": {"analysis_func": AsyncMock(), "display_func": MagicMock(),
                  "dependencies": [], "data_key": "whois", "description": "..."}
    }.get
...
async def test_orchestrator_quiet_mode(mock_table, mock_args):
    ...
    """
    mock_table.get.side_effect = {
        "records": {"analysis_func": AsyncMock(return_value={}),
                    "display_func": MagicMock(), "dependencies": [],
                    "data_key": "records", "description": "..."}
    }.get
