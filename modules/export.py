...
from .utils import get_desktop_path  # Keep for path resolution
...
def export_reports(all_data: Dict[str, Any]):
    ...
    if getattr(args, "html_file", None):  # This should be a path
    ...
    for file_type in set(output_files):  # Use set to avoid duplicates
        ...
        try:
            ...
            console.print(
                f"[green]✓ {output_format.upper()} report saved to:[/] {filepath}"
            )
        except ImportError:
            console.print(
                f"[bold red]Error: Export format '{output_format}' is not supported.[/bold red]"
            )
...
def handle_output(all_data: Dict[str, Any], mode: str):
    ...
    # --- 2. Handle Console Output (non-table formats) ---
        ...
        try:
            ...
            console.print(
                f"[bold red]Error: Console output format '{mode}' is not supported.[/bold red]"
            )
        except Exception as e:
            console.print(
                f"[bold red]An error occurred generating '{mode}' console output: {e}[/bold red]"
            )
