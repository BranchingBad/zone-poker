...
def export_reports(all_data: Dict[str, Any]):
    ...
    filename_template = getattr(args, 'filename_template',
                                '{domain}_dnsint_{timestamp}')
    ...
    for file_type in set(output_files):  # Use set to avoid duplicates
        ...
        try:
            ...
            output_module = importlib.import_module(f".output.{output_format}",
                                                    package="modules")
...
def handle_output(all_data: Dict[str, Any], mode: str):
    ...
    # --- 2. Handle Console Output (non-table formats) ---
        ...
        try:
            output_module = importlib.import_module(f".output.{mode}",
                                                    package="modules")
            output_module.output(all_data)
        except ImportError:
            console.print(
