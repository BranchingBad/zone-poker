...
def _evaluate_hsts(value: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if max_age < 31536000: # 1 year
            recommendations.append("HSTS 'max-age' is less than one year. Consider setting it to 31536000.")
        if not has_subdomains:
            recommendations.append("HSTS is missing the 'includeSubDomains' directive, leaving subdomains unprotected.")
        if not has_preload and has_subdomains and max_age >= 31536000:
            recommendations.append("Consider adding the 'preload' directive to HSTS for maximum protection.")

        status = "Strong" if max_age >= 31536000 and has_subdomains else "Weak"
        return {"status": status, "value": value, "recommendation": " ".join(recommendations)}
...
def _evaluate_xfo(value: str) -> Dict[str, Any]:
    ...
    if val_lower in ('deny', 'sameorigin'):
        return {"status": "Present", "value": value}
    return {"status": "Weak", "value": value, "recommendation": "X-Frame-Options should be set to 'DENY' or 'SAMEORIGIN'."}

def _evaluate_xxss(value: str) -> Dict[str, Any]:
    """Evaluates the X-XSS-Protection header."""
    if value.startswith('0'):
        return {"status": "Disabled", "value": value, "recommendation": "X-XSS-Protection is disabled. While modern browsers have built-in protection, ensure CSP is properly configured."}
    if value.startswith('1'):
        return {"status": "Enabled", "value": value, "recommendation": "X-XSS-Protection is enabled. Note that this header is deprecated in favor of a strong Content-Security-Policy."}
