...
def _evaluate_hsts(value: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not has_subdomains:
            recommendations.append(
                "HSTS is missing the 'includeSubDomains' directive, leaving subdomains unprotected."
            )
...
def _evaluate_xxss(value: str) -> Dict[str, Any]:
    ...
    if value.startswith("0"):
        return {
            "status": "Disabled",
            "value": value,
            "recommendation": "X-XSS-Protection is disabled. While modern browsers have built-in protection, ensure CSP is properly configured.",
        }
    if value.startswith("1"):
        return {
            "status": "Enabled",
            "value": value,
            "recommendation": "X-XSS-Protection is enabled. Note that this header is deprecated in favor of a strong Content-Security-Policy.",
        }
