...
async def email_security_analysis(domain: str, resolver: dns.resolver.Resolver, all_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    """
    ...
    spf_records = [r["value"] for r in records.get("TXT", []) if r.get("value", "").startswith("v=spf1")]
    if spf_records:
    ...
    try:
        ...
        if dmarc_records:
            ...
            if "p" not in analysis["dmarc"]:
                analysis["dmarc"]["warning"] = "DMARC record is missing the required 'p' (policy) tag."
            # Like SPF, there should only be one DMARC record.
            if len(dmarc_records) > 1:
                analysis["dmarc"]["warning"] = "Multiple DMARC records found. Only one is allowed."

    except (dns.resolver.NoAnswer, dns.resolver.NXDOMAIN, dns.exception.Timeout):
        analysis["dmarc"] = {"status": "Not Found"}

    # 3. DKIM - Informational
    analysis["dkim"] = {"status": "Cannot check without a selector (e.g., 'default._domainkey')."}
