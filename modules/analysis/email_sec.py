...
async def _get_spf_record(domain: str, resolver: dns.resolver.Resolver, depth: int = 0) -> List[str]:
...
async def _get_dmarc_record(domain: str, resolver: dns.resolver.Resolver) -> List[str]:
    ...
    try:
        ...
        return [join_txt_chunks([t.decode('utf-8', 'ignore') for t in rdata.strings]) for rdata in answers]
    except (dns.resolver.NoAnswer, dns.resolver.NXDOMAIN, dns.exception.Timeout, dns.resolver.NoNameservers):
        return []

async def email_security_analysis(domain: str, resolver: dns.resolver.Resolver, **kwargs) -> Dict[str, Any]:
    ...
    spf_records, dmarc_records = await asyncio.gather(
        _get_spf_record(domain, resolver),
        _get_dmarc_record(domain, resolver)
    )
