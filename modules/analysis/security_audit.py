...
def _audit_dns_records(audit: Dict, records_info: Dict, mail_info: Dict, nsinfo_info: Dict, zone_info: Dict):
    ...
    if "warning" in spf_data:
        ...
        _add_audit_entry(audit, "SPF Policy", "Weak", "Policy is '?all' (Neutral), which does not prevent spoofing.")
    elif spf_data.get("all_policy") == "~all":
        _add_audit_entry(audit, "SPF Policy", "Moderate", "Policy is '~all' (SoftFail), which is not fully secure.")
    elif spf_data.get("all_policy") == "-all":
        _add_audit_entry(audit, "SPF Policy", "Secure", "Policy is '-all' (HardFail), which is the recommended setting.")
    else:
        _add_audit_entry(audit, "SPF Policy", "Weak", "No SPF record or 'all' mechanism found.")
    ...
    if dmarc_data.get("p") == "none":
        ...
        _add_audit_entry(audit, "DMARC Policy", "Secure", f"Policy is '{dmarc_data['p']}', which protects against spoofing.")
    else:
        _add_audit_entry(audit, "DMARC Policy", "Weak", "No DMARC record found or policy is misconfigured.")

    # --- CAA Record ---
    if records_info.get("CAA"):
        _add_audit_entry(audit, "CAA Record", "Secure", "Present, restricting which CAs can issue certificates.")
    else:
        _add_audit_entry(audit, "CAA Record", "Weak", "Not found, allowing any Certificate Authority to issue certificates.")
    ...
    if "NSEC" in records_info:
        _add_audit_entry(audit, "DNSSEC Zone Walking", "Weak", "Uses NSEC, which allows for zone walking to enumerate all records.")
    elif "NSEC3" in records_info:
        _add_audit_entry(audit, "DNSSEC Zone Walking", "Secure", "Uses NSEC3, which prevents zone walking.")
...
def _audit_web_security(audit: Dict, headers_info: Dict, ssl_info: Dict, takeover_info: Dict):
    ...
    if missing_headers:
        _add_audit_entry(audit, "HTTP Headers", "Weak", f"Missing critical headers: {', '.join(missing_headers)}.")
    else:
        _add_audit_entry(audit, "HTTP Headers", "Secure", "All key security headers are present.")
    ...
    if hsts_analysis.get("status") == "Weak":
        _add_audit_entry(audit, "HSTS Policy", "Moderate", "HSTS is present but with a weak 'max-age' (< 1 year).")
    elif hsts_analysis.get("status") == "Strong":
        _add_audit_entry(audit, "HSTS Policy", "Secure", "HSTS is enabled with a strong policy.")

    # --- Content-Security-Policy ---
    csp_analysis = headers_info.get("analysis", {}).get("Content-Security-Policy", {})
    if csp_analysis.get("status") == "Missing":
        _add_audit_entry(audit, "CSP", "Weak", "Content-Security-Policy header is missing, increasing XSS risk.")
    else:
        _add_audit_entry(audit, "CSP", "Secure", "Content-Security-Policy header is present.")

    # --- SSL/TLS Certificate Validity ---
    if "error" in ssl_info:
        _add_audit_entry(audit, "SSL/TLS Certificate", "Weak", f"Could not be analyzed: {ssl_info['error']}")
    elif valid_until := ssl_info.get("valid_until"):
        if datetime.now().timestamp() > valid_until:
            _add_audit_entry(audit, "SSL/TLS Certificate", "Weak", "Certificate is expired.")
        else:
            _add_audit_entry(audit, "SSL/TLS Certificate", "Secure", "Certificate is valid.")

    # --- SSL/TLS Cipher Suites ---
    if cipher_info := ssl_info.get("cipher"):
        cipher_name = cipher_info[0]
        if any(weak_str in cipher_name for weak_str in WEAK_CIPHER_SUBSTRINGS):
            _add_audit_entry(audit, "SSL/TLS Ciphers", "Weak", f"Connection uses a weak cipher suite: {cipher_name}.")

    # --- Subdomain Takeover ---
    if vulnerable_takeovers := takeover_info.get("vulnerable", []):
        _add_audit_entry(audit, "Subdomain Takeover", "Vulnerable", f"Found {len(vulnerable_takeovers)} potentially vulnerable subdomains.")

def _audit_reputation_and_network(audit: Dict, dnsbl_info: Dict, port_scan_info: Dict, reputation_info: Dict):
    """Audits IP reputation and open network ports."""
    # --- DNSBL Listing ---
    if listed_ips := dnsbl_info.get("listed_ips", []):
        _add_audit_entry(audit, "IP Blocklist Status", "Weak", f"Found {len(listed_ips)} IP(s) on DNS blocklists.")

    # --- IP Reputation (AbuseIPDB) ---
    high_risk_ips = [
        ip for ip, info in reputation_info.items()
        if isinstance(info, dict) and info.get("abuseConfidenceScore", 0) > 75
    ]
    if high_risk_ips:
        _add_audit_entry(audit, "IP Reputation", "Weak", f"Found {len(high_risk_ips)} IP(s) with high abuse scores: {', '.join(high_risk_ips)}.")
    ...
    if open_ports_summary:
        _add_audit_entry(audit, "Open Ports", "Weak", f"Found open ports, which could increase attack surface. Details: {'; '.join(open_ports_summary)}")